...
local debug_hidden_nodes = false -- true
...
local function pairing_on_left (pos, door)
	...
	for l=1,max_pair_distance,1 do
		...
		if minetest.get_item_group(left_node_name, "door") == 1 then
			...
			local pair_width = door.size.width + door_sizes[string.sub(left_node_name,-6,-3)].width
			if (
...
local function pairing_on_right (pos, door)
	...
	for r=1,max_pair_distance,1 do
		...
		if minetest.get_item_group(right_node_name, "door") == 1 then
			...
			local pair_width = door.size.width + door_sizes[string.sub(right_node_name,-6,-3)].width
			if (
...
minetest.register_node("bigdoors:hidden", hidden_def) -- doors:hidden has a hitbox, no thanks
